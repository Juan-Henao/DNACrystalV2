...
public class ComandoControladorUsuarioTest {
	...
	public void actualizar() throws Exception {
		...
		// act - assert
		mocMvc.perform(put("/usuario/{id}", id).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(usuario))).andExpect(status().isOk());
	}
	...
	public void eliminar() throws Exception {
		...
		// act - assert
		mocMvc.perform(
				delete("/usuario/{id}", id).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
